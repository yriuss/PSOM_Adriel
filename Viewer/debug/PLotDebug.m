...
for cat = 1:length(Model.test.debug.histogramTrain{1})
    video = VideoWriter(['plots\histogram_cat_' int2str(cat) '.mp4'] , 'MPEG-4');
    open(video);
    nodes = 30;
    for t = 1:length(Model.test.debug.histogramTrain)-1
        ...
        bar(ratioTrain, 'm')
        hold on
        plot([1:epochs],nanmean(ratioTrain)*ones(epochs),'k--')
        hold off          
        axis([0 nodes 0 nanmax([ratioTrain; ratioTest])])
        xlabel('Nodes')
        ylabel('Ratio (Error/Hits)')
        title('Ratio Train')
        
        subplot(3,2,6)
        bar(ratioTest, 'm')
        hold on
        plot([1:epochs],nanmean(ratioTest)*ones(epochs),'k--')
        hold off          
        axis([0 nodes 0 nanmax([ratioTrain; ratioTest])])
        xlabel('Nodes')
        ylabel('Ratio (Error/Hits)')
