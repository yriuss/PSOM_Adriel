...
sMap.codebook = (0.02 *rand([munits dim])) - 0.01; % TODO 

numClass = 4;
label = {'KITCHEN'; 'BATHROOM'; 'BEDROOM'; 'OFFICE'};
means = [];
for i=1:numClass
    means = [means; mean(sData.data(SelectIndexs(sData, label(i)),:))];
end;

for i=1:numClass
    index = ceil((i-1)*(munits/numClass)+((munits/(2*numClass))));
    sMap.codebook(index,:) = means(i,:);
end;
