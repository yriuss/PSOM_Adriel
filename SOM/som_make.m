function [sMap,Model] = som_make(D, varargin)
...
while i<=length(varargin), 
  ...
  if ischar(varargin{i}), 
    switch varargin{i}, 
     ...
     case 'initrange',  i=i+1; sMap.range = varargin{i}; 
     case 'mult',       i=i+1; mult = varargin{i};    
     case 'dataselect',  i=i+1; sData.select = varargin{i};     
     case 'training',   i=i+1; training = varargin{i};
     case 'model',   i=i+1; Model = varargin{i};
     case 'dataTest', i=i+1; SamplesTest = varargin{i}; 
...
sMap = som_deepinit(D, sMap, 'model', Model, 'sData', sData);

%switch initalg, 
% case 'randinit', sMap = som_deepinit(D, sMap, 'model', Model, 'sData', sData);
% case 'prototypeinit', sMap = som_prototypeinit(sData, sMap);
...
%sTrain.trainlen = 3; TODO
switch func,
 case 'seq',    sMap = som_seqtrain(sMap,D,sTrain,'tracking',tracking,'mask',mask);
 case 'deep',   [sMap, ~, Model] = som_deeptrain(sMap,sData,D,sTrain,'tracking',tracking,'mask',mask,'model',Model,'SamplesTest',SamplesTest,'DeepSOM',DeepSOM);
