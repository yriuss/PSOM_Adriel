...
% %                             %Model.relevancies.b{i,j}(epoch,:) = 1 - iqr(sdataTrain.data(train_labels==i,:));
% %                         else
% %                             Model.relevancies.b{i,j}(epoch,:) = 1 - var(DeepSOM{i,layer-1}.BMUsValuesTrain(train_labels==i,:));
% %                             %Model.relevancies.b{i,j}(epoch,:) = 1 - iqr(DeepSOM{i,layer-1}.BMUsValuesTrain(train_labels==i,:));
% %                         end;
% %                         if strcmp(Model.multiple.relevanceNorm{1,layer},'max') || strcmp(Model.multiple.relevanceNorm{1,layer},'max_sum')
...
% %                     Model.relevancies.b{i,j}(epoch+1,:) = (alphaRelevance) * (Model.relevancies.r{i,j}(epoch+1,:) - Model.relevancies.r{i,j}(epoch,:)  ) + ...
% %                         (1-alphaRelevance) * (  Model.relevancies.r{i,j}(epoch,:) );                    
